...
class TreeForTD3(object):
    ...
    def do_step(self, action, fake=False):
        """
        input:
            action: concat(reattach_probs, prnoe_probs) size: 2* genes(mutations)_length
            fake  : does accept the given action to go to the next state
        output:
            new_obs, reward, done, _

        In this function we must choose prone and reattach points 
        and go to the next state.

        """

        # split action to prune and reattach probs
        prune_probs    = action[:len(self.gene_names)-1]
        reattach_probs = action[len(self.gene_names)-1:]

        # normalize prune probs
        prune_probs    = prune_probs/np.sum(prune_probs)

        # choose a node and its parent to catch a prune edge
        candidate_prune_nodes = self.gene_names.copy()
        candidate_prune_nodes.remove(self.root)
        choosed_prune  = np.random.choice(candidate_prune_nodes, 1 ,p=prune_probs)
        choosed_prune_parent = self.parent(choosed_prune)

        # prune the current tree
        new_T = self.__T.copy()
        new_T.remove_edge(choosed_prune_parent, choosed_prune)

        # select candidate nodes for choosing reattach point
        WCC = list(list(wc) for wc in nx.algorithms.components.weakly_connected_components(T))
        candidate_reattach_nodes = WCC[0] if choosed_prune_parent in WCC[0] else WCC[-1]

        # choose a node as a reattach point
        reattach_probs = [reattach_probs[int(n)] for n in candidate_reattach_nodes]
        reattach_probs = reattach_probs/np.sum(reattach_probs)
        choosed_reattach = np.random.choice(candidate_reattach_nodes, 1 ,p=reattach_probs)

        # reattach the tree
        new_T.add_edge(choosed_reattach, choosed_prune)

        # calculate new reward
        curr_error = self.__calc_tree_error(T=self.__T)
        next_error = self.__calc_tree_error(T=T)
        reward     = (curr_error - next_error) - 1 # minus: -> as a punishment to control delay

        # check if done
        done = next_error == 0

        # get and update new state
        if not fake:
            self.__T = new_T.copy()
            new_obs = self.get_current_obs()
        else:
            new_obs = self.get_current_obs(new_T)

        return new_obs, reward, done, _


    def get_E(self, T=None):
        return self.__get_E(T)


    def get_current_obs(T=None):
        new_obs = self.get_E(T)
        return new_obs




    def choose_prune_and_reattach_points(self, action):
        pass


    def choose_swap_points(self, action):
        pass
    ...
    def set_ground_truth(self, gt_D, gt_E, gt_T=None, gt_El=None):
        self.gt_D = gt_D
        self.gt_E = gt_E
        if gt_T:
            self.gt_T = gt_T
    ...
    def __plot_matrix(
        ...
        t = 1 ## alpha value
        if not cmap:
            cmap = {0:[1,1,0.95,t], 0.5:[0.9, 0, 0.3,t], 1:[0.5,0.5,0.8,t], -1:[0.8,0.5,0.5,t]}
        if not labels:
            labels = {0:'0', 0.5:'0.5', 1:'1', -1:'-1'}
    ...
    def __plot_EmGtE(self, DmE, title, filename=None):
        labels = {
            0:'true', 
            1:r'false positive (\alpha)', 
            -1:r'false negetive (\beta)', 
            0.5:r'lossed mutation',
            -0.5:r'lossed mutation'
        }
    ...
    def plot_all_results(self, plot_pm=False, p=5):
        ...
        if gt_D is not None and gt_E is not None:
            ...
            pdot.write_png(f'{self.save_dir}/example.png')
            img = mpimg.imread(f'{self.save_dir}/example.png')
        ...
        plt.savefig(f'{self.save_dir}/benchmark')
        plt.close()
    ...
    def plot_best_T(self, filename=None):
        ...
        # p.write_png(f'{self.save_dir}/example.png')
        # img = mpimg.imread(f'{self.save_dir}/example.png')
        # plt.figure(figsize=(20,20))
        # plt.imshow(img)
